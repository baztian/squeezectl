...
def main():
  ...
  parser.add_argument("--player", help="Id of the player/squeezebox")
  ...
  if known_args.player:
    server.update()
    try:
      player = server.player(known_args.player)
    except (KeyError) as e:
      print(server, file=stderr)
      raise Exception("Player {} not found".format(known_args.player)) from e
